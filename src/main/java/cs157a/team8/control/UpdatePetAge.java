 ...
 */
@WebServlet("/UpdatePetAge")
public class UpdatePetAge extends HttpServlet {
...
//            System.out.println("Edit Pet category: " + newPetCategory);
            ...
            if (newPetAge != null) {
                ...
                Pet pet = new Pet(petID, null, newPetAge, null, null); // Only the name changes

                // Update the user in the database using UserDao
                PetDao petDao = new PetDao();
                boolean isUpdated = petDao.updatePetAge(pet); // Update only the pet name
...
//                    session.setAttribute("petName", newName); // Update the session password with the new one
                ...
                response.getWriter().println("Pet Age cannot be empty!");
            }
        }
