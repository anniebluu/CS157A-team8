 ...
 */
@WebServlet("/UpdatePetCategory")
public class UpdatePetCategory extends HttpServlet {
...
//            System.out.println("Edit Pet age: " + newPetAge);
            ...
            if (newPetCategory != null) {
                // Create a User object with the new password (userID is from the session)
                Pet pet = new Pet(petID, null, null, newPetCategory, null); // Only the name changes

                // Update the user in the database using UserDao
                PetDao petDao = new PetDao();
                boolean isUpdated = petDao.updatePetCategory(pet); // Update only the pet name
...
//                    session.setAttribute("petName", newName); // Update the session password with the new one
                ...
                response.getWriter().println("Pet Category cannot be empty!");
            }
        }
